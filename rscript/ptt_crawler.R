...
##
content_re_crawler <- function(i, fail_list, ptt_df){
  tryCatch({
    url       <- ptt_df$Url[i]
    total_css <- read_html(url) 
    ...
    cat("\r Ptt article: ",i, " ==>", i/length(fail_list)*100, "% completed.",paste(replicate(50, " "), collapse = ""))
    Sys.sleep(runif(1, 4, 6))
    
  }, error = function(e) {
    cat("\n ")
    cat("\n", forum_name, " Ptt article: ", i, " failed. ", i/length(fail_list)*100, "%")
    Sys.sleep(runif(1, 4, 6))
  })
  return(ptt_df)
}
